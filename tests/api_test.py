...
class ApiTestClient:
    ...
    def test_clear_memory(self):
        ...
        # 14. Tool Calling 内容分析工具
    def test_content_analyze_tool(self):
        self.print_title("API-014 Tool Calling 内容分析工具")

        url = f"{BASE_URL}/ai/tools/content/analyze"
        payload = {
            "type": "追妻火葬场",
            "platform": "短视频漫剧",
            "audience": "18-25岁女性",
            "style": "强冲突、快节奏、反转",
            "text": "霸总误会女主，女主离开后男主追悔莫及"
        }

        response = requests.post(url, json=payload, headers=self.get_headers())
        self.print_response(response)

        self.assert_status_ok(response, "Tool Calling 内容分析工具")

        data = self.extract_data(response)
        output = data.get("output", {})

        assert data.get("toolType") == "CONTENT_ANALYZE", "内容分析工具失败：toolType 不正确"
        assert output.get("conflictLevel"), "内容分析工具失败：缺少 conflictLevel"
        assert output.get("emotionTone"), "内容分析工具失败：缺少 emotionTone"
        assert output.get("tags"), "内容分析工具失败：缺少 tags"
        assert output.get("suggestion"), "内容分析工具失败：缺少 suggestion"
    ...
    def test_content_agent_roundtable(self):
        ...
        url = f"{BASE_URL}/ai/agent/roundtable"
        payload = {
            "topic": "题材类型：追妻火葬场\n平台场景：短视频漫剧\n目标受众：18-25岁女性\n内容风格：强冲突、快节奏、反转\n具体需求：霸总误会女主，女主离开后男主追悔莫及，希望生成一个15集漫剧核心内容方案。",
            "agents": [
                {
                    "code": "PLOT",
                    "name": "剧情策划 Agent",
        ...
        self.assert_status_ok(response, "内容生产 Agent 圆桌")

        data = self.extract_data(response)

        assert data.get("mode") == "content_production", "内容生产圆桌失败：mode 不是 content_production"
        assert "agents" in data, "内容生产圆桌失败：缺少 agents 字段"
        assert "summary" in data, "内容生产圆桌失败：缺少 summary 字段"

        agents = data.get("agents", [])

        assert len(agents) == 4, "内容生产圆桌失败：Agent 数量不是 4"

        agent_codes = [item.get("agentCode") for item in agents]

        assert "PLOT" in agent_codes, "缺少剧情策划 Agent"
        assert "DIALOGUE" in agent_codes, "缺少角色对白 Agent"
        assert "RHYTHM" in agent_codes, "缺少情绪节奏 Agent"
        assert "OPS" in agent_codes, "缺少运营分析 Agent"

        print("✅ 内容生产 Agent 圆桌通过")
